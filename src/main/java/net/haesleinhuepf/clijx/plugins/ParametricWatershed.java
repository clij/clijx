 ...
 *         July 2020
 */
@Plugin(type = CLIJMacroPlugin.class, name = "CLIJx_parametricWatershed")
public class ParametricWatershed extends AbstractCLIJ2Plugin implements CLIJMacroPlugin, CLIJOpenCLProcessor, OffersDocumentation, IsCategorized, HasClassifiedInputOutput {
    @Override
    ...
    public boolean executeCL() {
        return parametricWatershed(getCLIJ2(), (ClearCLBuffer)( args[0]), (ClearCLBuffer)(args[1]), asFloat(args[2]), asFloat(args[3]), asFloat(args[4]));
    }

    public static boolean parametricWatershed(CLIJ2 clij2, ClearCLBuffer thresholded, ClearCLBuffer output, Float sigma_x, Float sigma_y, Float sigma_z ) {
        ClearCLBuffer distanceMap = clij2.create(thresholded.getDimensions(), NativeTypeEnum.Float);
        ...
        if (thresholded.getDimension() > 2 && thresholded.getDepth() > 1) {
            clij2.gaussianBlur3D(distanceMap, localMaxima, sigma_x, sigma_y, 0);
        } else {
            clij2.gaussianBlur3D(distanceMap, localMaxima, sigma_x, sigma_y, sigma_z);
        }
        clij2.mask(localMaxima, thresholded, distanceMap);
        ...
        dilateLabelsUntilNoChange(clij2, distanceMap, labelMap, distanceMap2, labelMap2);
    ...
    public String getParameterHelpText() {
        return "Image binary_source, ByRef Image destination, Number sigma_x, Number sigma_y, Number sigma_z";
    }

    @Override
    public String getDescription() {
        return "Apply a binary watershed to a binary image and introduce black pixels between objects.\n\n" +
